...
import { TableVirtualScrollStrategy } from './table-virtual-scroll-stragegy';
import { TableVirtualScrollDataSource } from './table-virtual-data-source';
...
export class TableItemSizeDirective implements OnChanges, AfterContentInit, OnDestroy {
    ...
    @Input() set tableComponent(table: MatTable<any>) {
        this.table = table;
        if (!!this.table) {
            this.init();
        }
        ...
        if (dataSource instanceof TableVirtualScrollDataSource) {
            ...
            dataSource
                ...
                .pipe(
                    ...
                    takeUntil(this.destroyed$)
                ).subscribe((data) => this.scrollStrategy.dataLength = data.length);
